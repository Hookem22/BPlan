...
public partial class Default : System.Web.UI.Page
{
    ...
    public static void UpdateProspect(Prospect prospect)
    {
        ...
        if (!string.IsNullOrEmpty(prospect.Email))
        {
            ...
            string body = string.Format("{0}, {1}", prospect.Name, prospect.Email);
            Email email = new Email("RestaurantBPlan@RestaurantBPlan.com", "williamallenparks@gmail.com", "New Subscriber", body);
            email.Send();
        }
    ...
    public static void SendNextEmail()
    {
        ...
        {
            if(prospect.LastEmailSent < DateTime.Now.AddHours(-23))
            {
                prospect.EmailSent++;
                prospect.LastEmailSent = DateTime.Now;
                prospect.Save();
